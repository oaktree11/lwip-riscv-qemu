...
printf("reif_ll_output\n");
	...
	for(i = 0, q = p; q != NULL; q = q->next) 
	{
		...
		{
		//#if DEBUG
			printf("reif_ll_output: pbuf %d . len: %d used: %d \n", i++, q->len, used);
	//	#endif
...
void reif_input(struct netif *netif, char *pkt, int len)
{
	...
	switch (htons(ethhdr->type)) 
	{
		...
		//case ETHTYPE_IPV6:
		case ETHTYPE_ARP:
			printf("ETHTYPE arp %x\n",htons(ethhdr->type));
			/* full packet send to tcpip_thread to process */
			int zx = ethernet_input1(p, netif);
			/*if (zx!=ERR_OK)//netif->input(p, netif) != ERR_OK)
			{ 
				printf("in netif->input\n");
...
static void reif_ll_init(struct netif *netif)
{
	...
	/* 0x12005452 52:54:00:12:34:56 set MAC address. XXX: this hardcode should be removed! */
	netif->hwaddr[0] = 0x52;
	netif->hwaddr[1] = 0x54;
	netif->hwaddr[2] = 0x00;
	netif->hwaddr[3] = 0x12;
